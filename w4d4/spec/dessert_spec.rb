...
describe Dessert do
  let(:chef) { double("chef", name: "Johann") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{Dessert.new("tart","fifty",chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["baking powder","chocolate","flour","vanilla"]
      ingredients.each do |ingredient| 
        cake.add_ingredient(ingredient)
      end
      cake.mix!
      expect(cake.ingredients).not_to eq(ingredients)
      expect(cake.ingredients.sort).to eq(ingredients)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Johann the Baker")
      expect(cake.serve).to eq("Chef Johann the Baker has made 10 cakes!")
    end
  end
